...
static void draw_text_bot(int x, int y, const char *s, uint8_t r, uint8_t g, uint8_t b)
{
    int n = (int)strlen(s);
    for (int i = 0; i < n; i++)
        draw_char_bot(x + i * 8, y, s[n - 1 - i], r, g, b);
}
...
static void draw_bot_help(const char *l1, const char *l2)
{
    ...
    if (l1) draw_text_bot_centered(100, l1, 200, 200, 210);
    if (l2) draw_text_bot_centered(120, l2, 140, 150, 170);
...
static void screen_about(void)
{
    ...
    drain();
    while (1) { if (wait_key() & BTN_B) return; }
}
...
static void screen_scripts(void)
{
    ...
    while (1) {
        ...
        if (k & BTN_A) {
            ...
            else if (sel == 1) o10_run(demo_info);
            else return;
        }
    }
}
