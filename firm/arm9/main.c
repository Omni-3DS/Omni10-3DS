...
{
    /* Bus 2 holds the MCU (device addr 0x4A) */
...
static bool i2c_write_mcu(uint8_t reg, uint8_t data)
{
    ...
    for (int tries = 0; tries < 8; tries++) {
        ...
        /* data + STOP (= 0xC1 like Luma) */
        regs->REG_I2C_DATA = data;
        regs->REG_I2C_CNT = I2C_ENABLE | I2C_IRQ_ENABLE | I2C_STOP;
        i2c_wait(regs);
        /* last ACK best-effort; MCU may already be resetting */
        return true;
    }
    return false;
}
...
{
    i2c_init();
    i2c_write_mcu(0x22, 1u << 0); /* LCD panel voltage off */
    for (volatile int i = 0; i < 300000; i++)
        __asm__ volatile("nop");
    i2c_write_mcu(0x20, 1u << 0); /* bit0 = power off */
    while (1)
        ;
}
...
{
    i2c_init();
    i2c_write_mcu(0x22, 1u << 0); /* LCD off */
    for (volatile int i = 0; i < 300000; i++)
        __asm__ volatile("nop");
