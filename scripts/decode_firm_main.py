...
# HID: ensure Y button (bit 11) for language toggle
if "BTN_Y" not in text:
    m = re.search(r"#define BTN_[A-Z]+[^\n]*\n", text)
    if m:
        text = text[: m.end()] + "#define BTN_Y (1u<<11)\n" + text[m.end() :]
    else:
        text = "#define BTN_Y (1u<<11)\n" + text
    print("injected BTN_Y")
if "g_lang" not in text:
    text = text.replace("static int g_ftp_on", "static int g_lang=0;\nstatic int g_ftp_on", 1)
    print("injected g_lang fallback")
